...
"""
classify_food.py
Single-image (or batch) food classifier using zero-shot CLIP.
- Free, local, CPU-friendly.
- Scalable structure (clear functions, CLI, top-k, JSON output, prompts).

Usage examples:
  python src/classify_food.py data/samples/banana.jpg
  python src/classify_food.py images/ --recursive --topk 3 --output json
  python src/classify_food.py *.jpg --labels data/labels.txt
  python src/classify_food.py img.jpg --labels-inline "banana,cabbage,bok choy"
"""
...
class Result:
    image: str
    predictions: List[Prediction]  # sorted by score desc
...
def expand_image_paths(
    ...
    for inp in inputs:
        ...
        if p.is_file():
            ...
            # support shell globs passed as literal strings (e.g., "*.jpg" on Windows PowerShell)
...
def build_prompts(labels: Sequence[str], template: str) -> List[str]:
    # You can enrich or branch per-label here later (synonyms, varieties, etc.)
...
def classify_one(
    ...
    try:
        ...
        # Return an empty prediction with an error message encoded in "label"
        return Result(
    ...
    with torch.no_grad():
        out = model(**inputs)
        probs = out.logits_per_image.softmax(dim=-1).squeeze(0)  # [num_labels]
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if output_mode == "json":
        ...
        # text mode:
        # - if topk==1: print "<label>" (single line per image)
        # - else: print "image\tlabel\tconfidence" for each prediction
        if int(args.topk) == 1:
            for r in results:
